...
def trainColorCorrectionModel(hp, tp, up, callbacks, upw):
        lr = tp['learning_rate']
        epochs = tp['epochs']
        ...
        rough_result_path = '/home/c/WangXia/codes/Modified/2019-RoughColorization9/test'
        guidance_image_path = '/home/c/WangXia/codes/AAAI2019/pictures/sceneflow/color_0_ycbcr'

        rough_color_imgs, guidance_imgs = extractAllImage(rough_result_path, guidance_image_path)
        train, val = splitData(rough_color_imgs, guidance_imgs, val_ratio, fraction)
        ...
        model = coloringnetwork.createColoringNetwork(hp, tp, upw)
        optimizer = optimizers.RMSprop(lr = lr, rho = rho, epsilon = epsilon, decay = decay)
        model.compile(optimizer = optimizer, loss = loss, metrics = [coloringLoss_OneAccuracy, coloringLoss_ThreeAccuracy])
...
if __name__ == '__main__':
        ...
        parser.add_argument('-upw', '--use_pretrained_weight', type = int, help = 'train the model use pretrained weight', default = 0)
        ...
        trainColorCorrectionModel(hp, tp, up, callbacks, args.use_pretrained_weight)
