...
def _computeSoftArgMin_(cv, d, ref_V, ref_U, GT_Y):
    softmax_weights = tf.nn.softmax(cv, dim = 1)#1,d,h,w
    max_d = d
...
def _LearnReg_(input, base_num_filters, ksize, ds_stride, resnet, padding, num_down_conv):
    down_convs = list()
    ...
    cost = _createDeconv3D_(up_convs, 1, ksize, 1, padding, bn = False, act_func = None)
...
def createColoringNetwork(hp, tp, pre_weight):
    ...
    layers = _createUniFeature_(input_shape, num_res, num_filters, first_ksize, ksize, act_func, padding)
    ...
    if shared_weight == 1:
        ...
        layers2 = _createUniFeature_(input_shape, num_res, num_filters, first_ksize, ksize, act_func, padding)
        r_feature = createFeature(right_img, layers2)

    unifeatures = [l_feature, r_feature]
    cv = Lambda(_getCostVolume_, arguments = {'max_d':d}, output_shape = (d, None, None, num_filters * 2))(unifeatures)
    disp_map = _LearnReg_(cv, num_filters, ksize, ds_stride, resnet, padding, num_down_conv)
    ...
    left_img_Y=left_img[:,:,:,2]
    right_img_V=right_img[:,:,:,0:1]
    right_img_U=right_img[:,:,:,1:2]
    output = Lambda(out_func, arguments = {'d':d,'ref_V':right_img_V,'ref_U':right_img_U,'GT_Y':left_img_Y})(disp_map_input)
